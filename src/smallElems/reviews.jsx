        ...
        <div className="review_container" style={{width:1216}}>  
            ...
            {reviews.map(review=>(
                <div className="single_review" key={review.id} style={{padding:'5px 40px',backgroundColor:'#1E1E1E',borderRadius:"10px",position:'relative'}}>
                     <h4 style={{fontSize:15,color:"gray"}} className="review_author">{review.author}</h4>
                     <p style={{color:'gray',fontSize:15,width:'100%'}}>{maxLength(review.content)}</p>
                     {show &&<button>no</button>}
                     <p style={{position:'absolute',top:14,right:40,color:"gray",fontSize:13}}>{review.created_at.slice(0,10).split('-').join('.')}</p>
                </div>
            ))}
           </div>
        </div>
